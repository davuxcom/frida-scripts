(function () {
    ...
    function CreateCLR() {
        function CallClrMethod(assemblyPath, typeName, methodName, params) {
            var Kernel32 = {
                LoadLibrary: new NativeFunction(Module.findExportByName("kernel32.dll", "LoadLibraryW"), 'pointer', ['pointer'], GetAbi()),
            };
            Kernel32.LoadLibrary(Memory.allocUtf16String("mscoree.dll"));
            var MSCorEE = {
                CLRCreateInstance: new NativeFunction(Module.findExportByName("MSCorEE.dll", "CLRCreateInstance"), 'uint', ['pointer', 'pointer', 'pointer'], GetAbi()),
            };

            var IEnumUnknown = new COM.Interface(COM.IUnknown, {
                Next: [0, ['ulong', 'pointer', 'pointer']],
            }, "00000100-0000-0000-C000-000000000046");

            var CLSID_CLRMetaHost = Win32.GUID.alloc("9280188d-0e8e-4867-b30c-7fa83884e8de");
            ...
            var CLSID_CLRRuntimeHost = Win32.GUID.alloc("90F1A06E-7712-4762-86B5-7A5EBA6BDB02");
            var ICLRRuntimeHost = new COM.Interface(COM.IUnknown, {
                Start: [0, []],
                ExecuteInDefaultAppDomain: [8, ['pointer', 'pointer', 'pointer', 'pointer', 'pointer']],
                /* virtual HRESULT STDMETHODCALLTYPE ExecuteInDefaultAppDomain(  [in]  LPCWSTR pwzAssemblyPath, [in] LPCWSTR pwzTypeName, [in] LPCWSTR pwzMethodName, [in] LPCWSTR pwzArgument, [out]  DWORD *pReturnValue) = 0; */
            ...
            var runtimeEnum = new COM.Pointer(IEnumUnknown);
            COM.ThrowIfFailed(metaHost.EnumerateInstalledRuntimes(runtimeEnum.GetAddressOf()));

            var runtimeInfo = null;
            while (true) {
                var arrayBuffer = new Struct({ 'element0': 'pointer' });
                var countWritten = new Struct({ 'value': 'ulong' });
                countWritten.value = 1;
                COM.ThrowIfFailed(runtimeEnum.Next(1, arrayBuffer.Get(), countWritten.Get()));

                runtimeInfo = new COM.Pointer(ICLRRuntimeInfo).Attach(arrayBuffer.element0);
                var char255 = Memory.alloc(255 * 2);
                var countWritten = new Struct({ 'value': 'ulong' });
                countWritten.value = 255;
                COM.ThrowIfFailed(runtimeInfo.GetVersionString(char255, countWritten.Get()));

                var runtimeStr = Memory.readUtf16String(char255);
                if (runtimeStr.indexOf("v4") > -1) { break; }
            }
            ...
            // Call the method.
            var retString = new Struct({ 'value': 'ulong' });
            COM.ThrowIfFailed(runtimeHost.ExecuteInDefaultAppDomain(Memory.allocUtf16String(assemblyPath),
                Memory.allocUtf16String(typeName), Memory.allocUtf16String(methodName), Memory.allocUtf16String(params), retString.Get()));
        ...
        this.New = function (typeName, args) {
            ...
            if (typeInfo.IsDelegate) {
                return ResolveResult(BridgeExports.CreateDelegate(Memory.allocUtf16String(typeName), DelegateCallback(args)));
            } else {
        ...
        var IClrBridge = new COM.Interface(COM.IUnknown, {
        ...
        var GetBridgeCallbackPtr = new NativeCallback(function (bridgePtr) {
            var bridge = new COM.Pointer(COM.IUnknown).Attach(bridgePtr).As(IClrBridge);
        ...
        // HostInfo.ProxyLibraryPath dependency
        COM.ThrowIfFailed(CallClrMethod(HostInfo.ProxyLibraryPath, "ClrBridge.ClrBridgeProvider", "GetBridge", "" + parseInt(GetBridgeCallbackPtr, 16)));
